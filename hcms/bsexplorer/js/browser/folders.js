        ...
        if (dir.parent().children('.folders').get(0)) {
            ...
            $('#loadingDirs').show(200, function() {
                $.ajax({
                    type: 'POST',
                    dataType: 'json',
                    url: browser.baseGetData('expand',addition),
                    data: {dir:path},
                    async: false,
...
browser.changeDir = function(dir) {
    if (dir.children('span.folder').hasClass('regular')) {
        ...
        $.ajax({
            type: 'POST',
            dataType: 'json',
            url: browser.baseGetData('chDir',addition),
...
browser.menuDir = function(dir, e) {
    ...
    $('.menu a[href="kcact:download"]').click(function() {
        browser.hideDialog();
        browser.post(browser.baseGetData('downloadDir',addition), {dir:data.path});
        return false;
    });
    ...
    $('.menu a[href="kcact:mkdir"]').click(function(e) {
        ...
        browser.fileNameDialog(
            e, {dir: data.path},
            'newDir', '', browser.baseGetData('newDir',addition), {
    ...
    $('.menu a[href="kcact:mvdir"]').click(function(e) {
        ...
        browser.fileNameDialog(
            e, {dir: data.path},
            'newName', data.name, browser.baseGetData('renameDir',addition), {
                title: "New folder name:",
                errEmpty: "Please enter new folder name.",
                errSlash: "Unallowable characters in folder name.",
                errDot: "Folder name shouldn't begins with '.'"
        ...
        browser.confirm(
            ...
            function(callBack) {
                 $.ajax({
                    type: 'POST',
                    dataType: 'json',
                    url: browser.baseGetData('deleteDir',addition),
                    data: {dir: data.path},
                    async: false,
                    success: function(data) {
