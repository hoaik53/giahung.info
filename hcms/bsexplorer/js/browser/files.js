    ...
    setTimeout(function() {
        ...
        $.each(browser.files, function(i, file) {
            ...
            if (_.kuki.get('view') == 'list') {
                ...
                if (file.thumb)
                    var icon = browser.baseGetData('thumb',addition) + '&file=' + encodeURIComponent(file.name) + '&dir=' + encodeURIComponent(browser.dir) + '&stamp=' + stamp;
...
browser.menuFile = function(file, e) {
    ...
    if (file.hasClass('selected') && files.length && (files.length > 1)) {
        ...
        $('.menu a[href="kcact:download"]').click(function() {
            ...
            browser.post(browser.baseGetData('downloadSelected',addition), {dir:browser.dir, files:pfiles});
            return false;
        });
        ...
        $('.menu a[href="kcact:rm"]').click(function() {
            ...
            var go = function(callBack) {
                ...
                $.ajax({
                    type: 'POST',
                    dataType: 'json',
                    url: browser.baseGetData('rm_cbd',addition),
                    data: {files:dfiles},
                    async: false,
        ...
        $('.menu a[href="kcact:download"]').click(function() {
            var html = '<form id="downloadForm" method="post" action="' + browser.baseGetData('download',addition) + '">' +
        ...
        $('.menu a[href="kcact:mv"]').click(function(e) {
            ...
            browser.fileNameDialog(
                ...
                'newName', data.name, browser.baseGetData('rename',addition), {
                    title: "New file name:",
            ...
            browser.confirm(browser.label("Are you sure you want to delete this file?"),
                function(callBack) {
                    $.ajax({
                        type: 'POST',
                        dataType: 'json',
                        url: browser.baseGetData('delete',addition),
